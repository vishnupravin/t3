...
const SearchScreen = ({ route }) => {
  ...
  const refreshME = () => {
    sql.getAllItem().then(res => {
      for (let sqlIndex = 0; sqlIndex < res.rows.length; sqlIndex++) {
        for (let catIndex = 0; catIndex < initalItems.length; catIndex++) {
          for (
            let itemIndex = 0;
            itemIndex < initalItems[catIndex].data.length;
            itemIndex++
          ) {
            if (
              initalItems[catIndex].data[itemIndex].id ==
              res.rows.item(sqlIndex).item_id
            ) {
              initalItems[catIndex].data[itemIndex].sqlid =
                res.rows.item(sqlIndex).id;
              initalItems[catIndex].data[itemIndex].qty =
                parseFloat(initalItems[catIndex].data[itemIndex].qty) +
                parseFloat(res.rows.item(sqlIndex).qty);
            }
          }
        }
      }
      // setAllMenuFromRedux(initalItems)
    });
    setRefresh(!refresh);
  ...
  const addingItem = async ({
    ...
    const ADDItem = ({ qty, type, isFree = false }) => {
      console.log({ catIndex, itemIndex, qty, price: parseInt(price), item, type, isFree })
                    ...
                    {itemModelItem.item.status == 0 ? (
                      ...
                      <View
                        ...
                        <Text
                          ...
                          {loadingItem != itemModelItem.item.id ? (
                            allMenuFromRedux[itemModelItem.catIndex]?.data[itemModelItem.itemIndex]?.qty
                          ) : (
                            <ActivityIndicator
                              size={
                                Math.round(Dimensions.get('screen').width) / 20
